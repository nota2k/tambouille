...
function createServiceMock() {
  return {
    ...
    create: jest.fn(async (userId: string, dto: any, files: { audioUrl?: string; coverUrl?: string }) => {
      assertExactlyOneAudioSource(files.audioUrl || null, dto.mixcloudKey || null);
...
const COVER_SOURCE_URL = 'https://thumbnailer.mixcloud.com/unsafe/1024x1024/cover.jpg';
...
describe('MixesController', () => {
  ...
  describe('create — nothing is written before the audio source is known', () => {
    it('rejects a mix with neither source without importing its cover', async () => {
      const create = controller.create(USER_ID, { title: 'A mix', coverSourceUrl: COVER_SOURCE_URL }, {});

      await expect(create).rejects.toBeInstanceOf(BadRequestException);
      await expect(create).rejects.toThrow('A mix must have either an audio file or a Mixcloud key');
    ...
    it('rejects a mix with both sources without importing its cover', async () => {
      const create = controller.create(
        USER_ID,
        { title: 'A mix', mixcloudKey: MIXCLOUD_KEY, coverSourceUrl: COVER_SOURCE_URL },
        { audio: [uploadedFile('audio/track.mp3')] },
      );

      await expect(create).rejects.toThrow('A mix cannot have both an audio file and a Mixcloud key');
      expect(coverImport.importFromUrl).not.toHaveBeenCalled();
    });

    it('still imports the cover for a Mixcloud-hosted mix, which has no audio file', async () => {
      await controller.create(USER_ID, { title: 'A mix', mixcloudKey: MIXCLOUD_KEY, coverSourceUrl: COVER_SOURCE_URL }, {});

      expect(coverImport.importFromUrl).toHaveBeenCalledWith(COVER_SOURCE_URL);
      expect(mixesService.create).toHaveBeenCalledWith(
        USER_ID,
        expect.objectContaining({ mixcloudKey: MIXCLOUD_KEY }),
    ...
    it('still imports the cover for an uploaded mix, and passes the audio key on', async () => {
      ...
      expect(coverImport.importFromUrl).toHaveBeenCalledWith(COVER_SOURCE_URL);
      expect(mixesService.create).toHaveBeenCalledWith(USER_ID, expect.anything(), {
    ...
    it('prefers an uploaded cover over an imported one, importing nothing', async () => {
      await controller.create(
        USER_ID,
        { title: 'A mix', coverSourceUrl: COVER_SOURCE_URL },
        { audio: [uploadedFile('audio/track.mp3')], cover: [uploadedFile('covers/uploaded.jpg')] },
      );
