 ...
 *
 * Reste LYL Radio, dont le fichier s'appelle `CHRISTIAN_COIFFURE_JUILLET_
 * da91f5c2f0.mp3` et ne dit donc rien de son épisode. Son API sait filtrer sur
 * l'adresse du fichier, ce qui rend l'épisode en une requête : c'est le seul
 * appel réseau de ce script.
 *
 * ── Ce qu'il ne retrouve pas ───────────────────────────────────────────────
 *
 * The Brain Radioshow. Ses pages sont numérotées (`listen.php?episode=213`) et
 * rien, ni sur l'accueil ni ailleurs, ne relie un mp3 à son numéro : il
 * faudrait balayer les pages une à une jusqu'à tomber sur la bonne. Aucun mix
 * The Brain n'est concerné aujourd'hui, et le jour où il y en aura, le
 * réimporter depuis sa page coûte moins qu'un balayage. Le script les nomme
 * dans son rapport plutôt que de faire semblant.
 *
 * ── En production ──────────────────────────────────────────────────────────
 *
 ...
 *
 * Exportée pour la boucle de `main` seule — la déduction et la recherche LYL
 * ont chacune leur spec, celle-ci ne fait que les enchaîner.
 */
export async function pageDeLaLigne(
  mix: Ligne,
  lireLyl: LecteurJson = lireApi,
): Promise<string | null> {
  ...
  if (LYL_HOSTS.includes(host)) {
    return pageLylDepuisAudio(mix.sourceRef, lireLyl);
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const mix of mixes) {
      const page = await pageDeLaLigne(mix);
      if (!page) {
        restants.push(mix);
        continue;
      }
