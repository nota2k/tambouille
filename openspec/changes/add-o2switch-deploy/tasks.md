...
- [ ] 1.1 Créer une paire de clés SSH **dédiée au déploiement**, distincte de toute clé personnelle, et déposer la publique dans cPanel › Accès SSH. Une clé dédiée se révoque sans rien casser d'autre.
...
- [ ] 1.3 Vérifier depuis un runner que rclone atteint le serveur en SFTP et **liste** `~/tambouille` sans rien écrire (`rclone lsd`). C'est la première fois que le transfert est exercé ; qu'il soit en lecture seule est délibéré.

## 2. La tâche `deploy`

- [ ] 2.1 Ajouter la tâche `deploy` à `.github/workflows/ci.yml` : `needs: [backend, frontend, e2e]`, déclencheur `workflow_dispatch` avec une entrée `ref` facultative, et une condition qui l'empêche de s'exécuter sur une `pull_request`. *Vérifier* : ouvrir une PR quelconque et constater que seules trois tâches apparaissent.
- [ ] 2.2 Installer rclone par `apt-get` et construire les artefacts dans la tâche : `npm ci` puis `npm run build` des deux paquets, plus `prisma generate`. Pas d'action tierce, pas d'`upload-artifact` — voir `design.md`, Decisions.
- [ ] 2.3 Écrire le transfert : `rclone sync` sur `backend/dist/`, `backend/generated/`, `backend/prisma/`, `frontend/dist/`, chacun avec son `--max-delete` ; `rclone copy` pour `package.json` et `package-lock.json`. **Aucune commande ne vise `backend/` lui-même.** Configuration par variables `RCLONE_SFTP_*`, pas par fichier encodé dans un secret.
...
- [ ] 2.7 Écrire l'étape SSH : activation du `nodevenv`, `node -v` **dont la sortie est vérifiée à 22.x**, `npx --yes prisma@7 migrate deploy`, puis `touch backend/tmp/restart.txt`. Migrer avant de redémarrer, jamais l'inverse.
...
- [ ] 2.9 Ajouter la vérification d'après déploiement : une requête sur l'API et une sur le site, l'échec de l'une faisant échouer la tâche.
