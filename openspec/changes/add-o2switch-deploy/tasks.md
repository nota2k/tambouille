...
- [x] 1.3 **Fait, et le cantonnement vérifié** : la racine du compte FTP liste `backend`, `frontend`, `openspec`, `TODOS.md` — c'est bien `~/tambouille` et rien au-dessus, donc ce compte ne peut pas atteindre `~/bin/`. Trois secrets posés : `O2SWITCH_FTP_HOST`, `O2SWITCH_FTP_USER`, `O2SWITCH_FTP_PASS`. Créer dans cPanel un **compte FTP dédié**, cantonné à `/home/<compte>/tambouille`. Le cantonnement n'est pas du confort : combiné au fait que le script du cron vit dans `~/bin/`, c'est lui qui empêche un identifiant de dépôt de fichiers de valoir exécution de code. Enregistrer `O2SWITCH_FTP_USER` et `O2SWITCH_FTP_PASS`.
- [x] 1.4 **Fait, en quatre essais dont trois échecs instructifs.** Le certificat du serveur n'est valide que pour `raisin.o2switch.net` et ses sous-domaines, pas pour l'alias `ftp.bane2718.odns.fr` — c'est le message d'erreur lui-même qui a donné le bon nom. Puis trois `530 Login authentication failed` : ni saut de ligne parasite ni interpolation shell, mes deux hypothèses étaient fausses ; une simple longueur affichée a montré que le secret du nom d'utilisateur contenait 10 caractères là où il en faut 38. Le TLS est bien actif, la validation du certificat n'est pas désactivée. Vérifier depuis un runner que rclone atteint le serveur en **FTPS explicite** et liste `~/tambouille` sans rien écrire. Le mode `dry_run` du workflow fait exactement cela. *Vérifier aussi* que le TLS est bien actif — un FTP nu ferait voyager le mot de passe et tous les fichiers en clair, ce qui serait un recul considérable par rapport à la clé SSH qu'on abandonne.
...
- [x] 2b.4 **Les deux moitiés établies.** Passage nominal : 20 suppressions annoncées, toutes sous `frontend/dist/assets` — exactement les fichiers construits sur le serveur le 9 août — et **aucune sous `backend/`**, donc `.env`, `node_modules` et les 222 Mo d'`uploads` sont hors d'atteinte. Puis, seuil abaissé à 1 : `Got fatal error on delete: --max-delete threshold reached`, étape en échec, rien d'effacé. Seuil restauré et passage revérifié au vert. **Exercer la garde avant d'en dépendre** : lancer en `dry_run` et lire ce que chaque `sync` annonce supprimer ; puis, en abaissant temporairement un `--max-delete` à 1, constater que l'opération **s'interrompt** au lieu d'effacer. C'est le scénario « Portée de transfert erronée », et la seule tâche qui distingue une garde d'un commentaire rassurant.
